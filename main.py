...
class SpriteAnimator(QLabel):
    ...
    def load_spritesheet(
        self, filepath, cols=COLS, rows=ROWS, target_height=250, frame_indices=None
    ):
        base, ext = os.path.splitext(filepath)
        if not base.endswith("_transparent"):
            transparent_path = f"{base}_transparent{ext}"
        ...
        if frame_indices is not None:
            self.frames = [
                self.frames[i] for i in frame_indices if i < len(self.frames)
            ]
...
class BubbleWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        path = QPainterPath()
        path.addRoundedRect(
            0, 0, float(rect.width()), float(rect.height() - 15), 20.0, 20.0
        )
...
class MonsterDeleter(QWidget):
    ...
    def mousePressEvent(self, event):
        if (
            not self.monster_sequence_started
    ...
    def start_phase1_walk(self):
        ...
        self.animator.load_spritesheet(
            os.path.join(SPRITE_DIR, "走路动效_spritesheet_transparent.webp")
        )

        screen = QApplication.primaryScreen().geometry()
        start_x = -self.animator.width()
        start_y = (
            self.target_pos.y() - self.animator.height() // 2 + 50
        )  # Move pet down
    ...
    def start_phase2_point(self):
        ...
        self.animator.load_spritesheet(
            os.path.join(SPRITE_DIR, "指着文件_spritesheet_transparent.webp"),
            frame_indices=[11, 12, 13, 14],
        )
    ...
    def start_phase3_kick(self):
        ...
        self.animator.load_spritesheet(
            os.path.join(SPRITE_DIR, "踹文件动效_spritesheet_transparent.webp")
        )
    ...
    def trigger_explosion(self):
        ...
        self.explosion_animator.load_spritesheet(
            os.path.join(SPRITE_DIR, "爆炸_spritesheet_transparent.webp"),
            target_height=150,
        )
    ...
    def start_phase4_leo(self):
        self.animator.load_spritesheet(
            os.path.join(SPRITE_DIR, "雷欧登场_spritesheet_transparent.webp")
        )
    ...
    def start_phase5_fly(self):
        ...
        self.animator.load_spritesheet(
            os.path.join(SPRITE_DIR, "出场飞行动效_spritesheet_transparent.webp")
        )
        self.animator.play(fps=8, loop=True)
